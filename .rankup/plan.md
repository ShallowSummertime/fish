...
| P1 | Money 与 Cooking 任务页 | done | 只使用已验证 grill/cooking/Killscore 规则；不宣传未复现的 casino 收益路线；线上 1190 词、4 图及 Article 技术信号通过 |
| P1 | Tuna creature 详情页 | done | 覆盖捕获准备、完整鱼体处理、Albatross 触发关系与失败恢复；线上 931 词、4 图及 Article 技术信号通过 |
| P1 | Bowhead Whale creature 详情页 | done | 区分普通 Bowhead 任务鱼与 Mutated Boss，覆盖 Scientist、Fish Bucket、完整搬运与火山口交付；线上 924 词、4 图及 Article 技术信号通过 |
| P1 | 遗失 Boss 任务物与 Boss 不刷新排错页 | done | 覆盖 Giant Piranha Skeleton、Pufferfish Fin、Albatross Head、Whale Fin 的路线状态检查和恢复动作；线上 1166 词、5 图及 Article 技术信号通过 |
| P1 | 首批高证据 creature 详情页 | in-progress | Tuna 与 Bowhead Whale 已生产发布；其余仅选择有独立素材与路线用途的实体，每页必须有信息增益，禁止复制数据库字段成模板薄页 |
